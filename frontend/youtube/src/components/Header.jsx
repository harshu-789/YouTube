    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm">
      <div className="flex items-center justify-between px-4 py-2">
        <div className="flex items-center gap-4">
          ...
          <Link to="/" className="flex items-center gap-1">
            <img src="https://www.youtube.com/s/desktop/7c155e84/img/favicon_144x144.png" alt="YouTube" className="h-8" />
            <span className="text-xl font-semibold">YouTube</span>
          </Link>
        </div>
        <form onSubmit={handleSubmit} className="flex items-center flex-1 max-w-2xl mx-4">
          ...
          />
          <button type="submit" className="px-6 py-2 bg-gray-100 border border-l-0 border-gray-300 rounded-r-full hover:bg-gray-200">
            <MagnifyingGlassIcon className="h-6 w-6" />
          </button>
        </form>
        <div>
          {user ? (
            <div className="flex items-center gap-4">
              <span>{user.username}</span>
              <button onClick={() => dispatch(logout())} className="px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-lg">
                Logout
              </button>
            </div>
          ) : (
            <Link to="/auth" className="px-4 py-2 text-blue-600 border border-blue-600 rounded-lg hover:bg-blue-50">
              Sign In
            </Link>
          )}
        </div>
